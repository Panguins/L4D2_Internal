...
#pragma warning( pop )
	...
	class c_window : public c_control
	{
		...
		void draw_watermark( float x, float y, float w, float h )
		{
			...
			std::string time = xor ( __DATE__ );
			std::string username = xor ( "SenatorII" );
			std::string watermark = time + xor ( " | " ) + username;
	...
	public:
		...
		bool handle( std::vector<std::string>& tabs, UIcolor_t color )
		{
			...
			if ( g_ctx.m_settings.menu_fade_speed > 0 )
				m_data.m_alpha = std::clamp<int>( m_should_open ? ( m_data.m_alpha + g_ctx.m_settings.menu_fade_speed ) : ( m_data.m_alpha - g_ctx.m_settings.menu_fade_speed ), 0, 255 );
			else
				m_data.m_alpha = std::clamp<int>( m_should_open ? ( m_data.m_alpha + 255 ) : ( m_data.m_alpha - 255 ), 0, 255 ); // sorry

			if ( g_ctx.m_settings.menu_rainbow )
				g_ctx.m_settings.menu_color( ) = ( m_data.m_color = UI::UIcolor_t::get_rainbow( m_data.m_alpha ) ).to_renderer_color( );
			else
	...
	public:
		...
		void end( c_window_data* data, const char* text )
		{
			m_text = text;
	...
	public:
		...
		void draw( c_window_data* data )
		{
			...
			if ( m_is_waiting )
				UI::string( data->m_color, m_box_start.x + 10.0f, m_box_start.y + 3.f, false, "press key" );
			else if ( *m_key )
